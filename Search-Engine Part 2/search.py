...
def unique_authors(count, metadata):
    ...
    author = []
    for article in metadata:
        if article[1].lower() not in author:
            res.append(article)
            author.append(article[1].lower())
    res = res[:count]
    return res
...
def most_recent_article(metadata):
    ...
    if len(metadata) == 0:
        ...
        for i in range(len(metadata)):
            if metadata[i][2] > recent_article:
                recent_article = metadata[i][2]
                article_index = i
        return metadata[article_index]   
...
def title_and_author(metadata):
    res = []
    for article in metadata:
        res.append((article[0], article[1]))
    return res     
# 7) 
