...
def article_count(count, titles):
    result = []
    if count <= len(titles):
        result.extend(titles[:count])
    elif count > len(titles):
        result.extend(titles[:len(titles)]) 
    return result 
# 4) 
#
...
def random_article(index, titles):
    invalid_result = ''
    if index == -1 or index > len(titles) - 1:
        return invalid_result
    elif index != -1 and index <= len(titles) - 1:
        return titles[index] 
    elif len(titles) == 0:
        return invalid_result
